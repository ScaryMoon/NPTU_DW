    ...
    methods: {
        create(){
            ...
            axios.get(apiUrl).then(res=>{
                ...
                if (this.BeforeSearchMovie != []){
                    this.BeforeSearchMovie.forEach((e)=>{
        ...
        foundORnot(times){
            if(times==0){
                this.BeforeSearchMovie=this.movie
            }
            // if(this.Nsearch!='' &&times==0){
            //     this.movies.forEach((m)=>{
            //         // console.log(this.movies)
            //         if(m.name.includes(this.Nsearch)!=true){
            //             var inde=this.movies.indexOf(m)
            //             this.movies.splice(inde,1)
            //             this.foundORnot(1) //因為被刪掉之後全部左位移 所以indexOf後的東西都不一樣了
